<template>
  <div>
    ...
    <!--Stats cards-->
    <div class="row">
      <div class="col-md-6 col-xl-2" v-for="(opcao, index) in cards" :key="opcao.id">
        <stats-card>
    ...
    <div class="row">
      <div class="col-12">
        <apexchart width="45%" :options="chartOptions" :series="series"></apexchart>
      </div>
    </div>
  ...
  data() {
    return {
      ...
      chartOptions: {
        ...
        fill: {
          colors: ["rgb(50,205,50)", "rgb(255,255,0)", "rgb(255,0,0)"]
        }
      },
      series: [],
      icones: [
        ...
        Object.keys(this.perguntaList).forEach(function(item) {
          ...
          Object.keys(self.perguntaList[item]).forEach(function(inner) {
            if (inner == self.pergunta) {
              total += self.perguntaList[item][inner];
              self.cards.push({
                descricao: self.traducaoOpc[item],
                valor: total
              });
            }
          });
